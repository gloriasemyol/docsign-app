...
export default function PDFEditor() {
  ...
  const [signerEmail, setSignerEmail] = useState(''); // <-- Added state for the email input
  ...
  // NEW FUNCTION: Sends the invite token to the backend
  const sendInvitation = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      // Calls your backend Step 4 route: /api/signatures/invite
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/signatures/invite`,
        { 
          documentId: docId, 
      ...
      alert(`Invitation generated! Token: ${res.data.token}\n\nLink: https://docsign-app-gilt.vercel.app/sign/${res.data.token}`);
      console.log("Generated Link:", `https://docsign-app-gilt.vercel.app/sign/${res.data.token}`);
    } catch (err) {
